...
fn solve<W: std::io::Write>(src: &str, out: &mut W) {
    ...
    let mut r = r1;
    let mut c = c1;
    while !can_big_move(r, c, r2, c2) {}
    while (r2 - r).abs() + (c2 - c).abs() > 3 {
        let r_range = r2 - r;
        let c_range = c2 - c;
        let range = ((r_range.pow(2) + c_range.pow(2)) as f64).sqrt() as isize;

        if r_range % c_range == 0 || c_range % r_range == 0 && (r_range > 1 && c_range > 1) {
            ans += 1;
            r += r_range;
            c += c_range;
...
fn can_big_move(r1: isize, c1: isize, r2: isize, c2: isize) -> bool {
